...
const exportPatientPDF=(p,plans,notes,poids,mens)=>{
    const win=window.open("","_blank");
    const bmi=calcBMI(p.poids,p.taille);
    const mb=calcMB(p);
    const det=calcDET(p);
    const moralEmojis=["","😞","😕","😐","🙂","😄"];
    const poidsRows=(poids||[]).map(d=>"<tr><td>"+new Date(d.date).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})+"</td><td><strong>"+d.poids+" kg</strong></td><td style='color:#8A7968;font-size:11px'>"+(d.note||"-")+"</td></tr>").join("");
    const mensRows=(mens||[]).map(d=>"<tr><td>"+new Date(d.date).toLocaleDateString("fr-FR",{day:"numeric",month:"short",year:"numeric"})+"</td><td>"+(d.nombril?d.nombril+"cm":"-")+"</td><td>"+(d.hanche?d.hanche+"cm":"-")+"</td><td>"+(d.cuisse_d?d.cuisse_d+"cm":"-")+"</td><td>"+(d.bras_d?d.bras_d+"cm":"-")+"</td><td>"+(d.masse_grasse?d.masse_grasse+"%":"-")+"</td><td>"+(d.masse_musculaire?d.masse_musculaire+"%":"-")+"</td><td>"+(d.masse_hydrique?d.masse_hydrique+"%":"-")+"</td></tr>").join("");
    const plansHtml=(plans||[]).map((plan,i)=>"<div style='margin-bottom:20px'><div style='background:#C4956A;color:white;padding:8px 14px;border-radius:6px 6px 0 0;font-weight:600;font-size:13px'>Plan "+(i+1)+" - "+new Date(plan.created_at).toLocaleDateString("fr-FR")+" ("+plan.duration+" - "+(plan.mode==="manual"?"Manuel":"IA")+")</div><div style='border:1px solid #E8DDD0;border-top:none;padding:12px 14px;border-radius:0 0 6px 6px'>"+(plan.days||[]).map(day=>"<div style='margin-bottom:10px'><div style='font-weight:600;color:#8B5E3C;font-size:11px;text-transform:uppercase;margin-bottom:4px'>"+day.label+"</div>"+(day.meals||[]).filter(m=>m.content).map(m=>"<div style='padding:4px 0;border-bottom:1px solid #f5f5f5;font-size:12px'><strong style='color:#8A7968'>"+m.name+(m.grammage?" ("+m.grammage+")":"")+" :</strong> "+m.content+"</div>").join("")+"</div>").join("")+(plan.tips?"<div style='background:#f0f7f2;border-left:3px solid #7A9E7E;padding:8px 12px;margin-top:8px;font-size:12px'>Conseils : "+plan.tips+"</div>":"")+"</div></div>").join("");
    const notesHtml=(notes||[]).map(n=>"<div style='border-left:3px solid #7A9E7E;padding:8px 12px;margin-bottom:8px;background:#f9f9f9;border-radius:0 6px 6px 0'><div style='font-size:10px;color:#8A7968;margin-bottom:4px'>"+new Date(n.created_at).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})+"</div><div style='font-size:12px;white-space:pre-line'>"+n.text+"</div></div>").join("");
    const bilanHtml=(p.sommeil||p.transit||p.moral||p.alimentation)?"<h2>Bilan sante initial</h2><div class='grid4'>"+(p.sommeil?"<div class='stat'><div class='stat-val'>"+(SOMMEIL_FR[p.sommeil]||p.sommeil)+"</div><div class='stat-label'>Sommeil</div></div>":"")+(p.transit?"<div class='stat'><div class='stat-val'>"+(TRANSIT_FR[p.transit]||p.transit)+"</div><div class='stat-label'>Transit</div></div>":"")+(p.moral?"<div class='stat'><div class='stat-val'>"+p.moral+"/5 "+(moralEmojis[p.moral]||"")+"</div><div class='stat-label'>Moral</div></div>":"")+(p.alimentation?"<div class='stat'><div class='stat-val'>"+(ALIM_FR[p.alimentation]||p.alimentation)+"</div><div class='stat-label'>Alimentation</div></div>":"")+"</div>"+(p.prise_de_sang?"<div style='background:#f5f8ff;border:1px solid #c0c8f5;border-radius:6px;padding:8px 12px;font-size:12px;margin-bottom:10px'><strong>Prise de sang :</strong> "+p.prise_de_sang+"</div>":""):"";
    const css="*{box-sizing:border-box;margin:0;padding:0}body{font-family:Georgia,serif;max-width:820px;margin:0 auto;color:#3D3228;padding:30px 40px;font-size:13px}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;padding-bottom:16px;border-bottom:2px solid #C4956A}.logo{font-size:22px;font-weight:bold;color:#C4956A}.logo-sub{font-size:9px;color:#8A7968;text-transform:uppercase;letter-spacing:2px;margin-top:2px}.patient-name{font-size:24px;color:#2A2118;margin-bottom:4px}.patient-info{font-size:12px;color:#8A7968}h2{font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#C4956A;margin:22px 0 10px;padding-bottom:5px;border-bottom:1px solid #E8DDD0}.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px}.stat{background:#FAF7F2;border:1px solid #E8DDD0;border-radius:8px;padding:10px;text-align:center}.stat-val{font-size:15px;font-weight:600;color:#2A2118}.stat-label{font-size:9px;color:#8A7968;text-transform:uppercase;letter-spacing:1px;margin-top:2px}table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:12px}th{background:#F0EBE1;padding:6px 10px;text-align:left;font-size:10px;text-transform:uppercase;color:#8A7968}td{padding:6px 10px;border-bottom:1px solid #F0EBE1}.alert{background:#fff8f5;border:1px solid #f5c0b8;border-radius:6px;padding:8px 12px;font-size:12px;margin-bottom:10px}.badge{display:inline-block;padding:2px 8px;border-radius:20px;font-size:10px;font-weight:500;background:rgba(61,90,71,0.12);color:#3D5A47;margin-right:4px}@media print{button{display:none}}";
    win.document.write("<!DOCTYPE html><html><head><title>Dossier - "+p.prenom+" "+p.nom+"</title><style>"+css+"</style></head><body>"+
    "<div class='header'><div><div class='patient-name'>"+p.prenom+" "+p.nom+"</div><div class='patient-info'>"+getAge(p.ddn)+(p.sexe?" - "+(p.sexe==="F"?"Femme":"Homme"):"")+(p.email?" - "+p.email:"")+(p.tel?" - "+p.tel:"")+"</div><div style='margin-top:8px'>"+(p.objectif?"<span class='badge'>"+GOALS_FR[p.objectif]+"</span>":"")+(p.diets||[]).map(d=>"<span class='badge' style='background:rgba(196,149,106,0.14);color:#8B5E3C'>"+DIET_FR[d]+"</span>").join("")+"</div></div>"+
    "<div style='text-align:right'><div class='logo'>SoDiet</div><div class='logo-sub'>Espace praticien</div><div style='font-size:10px;color:#8A7968;margin-top:6px'>"+new Date().toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})+"</div></div></div>"+
    "<h2>Donnees morphologiques</h2><div class='grid4'>"+
    "<div class='stat'><div class='stat-val'>"+(p.taille||"-")+" cm</div><div class='stat-label'>Taille</div></div>"+
    "<div class='stat'><div class='stat-val'>"+(p.poids||"-")+" kg</div><div class='stat-label'>Poids initial</div></div>"+
    "<div class='stat'><div class='stat-val'>"+(p.poids_obj||"-")+" kg</div><div class='stat-label'>Objectif</div></div>"+
    "<div class='stat'><div class='stat-val'>"+bmi+"</div><div class='stat-label'>IMC</div></div>"+
    "<div class='stat'><div class='stat-val'>"+(mb?mb+" kcal":"-")+"</div><div class='stat-label'>Metabolisme base</div></div>"+
    "<div class='stat'><div class='stat-val'>"+(det?det+" kcal":"-")+"</div><div class='stat-label'>Depenses totales</div></div>"+
    "<div class='stat'><div class='stat-val'>"+(ACTIVITE_FR[p.activite]||"-")+"</div><div class='stat-label'>Activite</div></div>"+
    "<div class='stat'><div class='stat-val'>"+(poids||[]).length+"</div><div class='stat-label'>Mesures poids</div></div></div>"+
    (p.allergies?"<div class='alert'><strong>Allergies :</strong> "+p.allergies+"</div>":"")+
    (p.antecedents?"<div class='alert' style='background:#f5f8ff;border-color:#c0c8f5'><strong>Antecedents :</strong> "+p.antecedents+"</div>":"")+
    bilanHtml+
    (poidsRows?"<h2>Suivi du poids</h2><table><thead><tr><th>Date</th><th>Poids</th><th>Note</th></tr></thead><tbody>"+poidsRows+"</tbody></table>":"")+
    (mensRows?"<h2>Mensurations et composition corporelle</h2><table><thead><tr><th>Date</th><th>Nombril</th><th>Hanche</th><th>Cuisse D</th><th>Bras D</th><th>MG%</th><th>MM%</th><th>MH%</th></tr></thead><tbody>"+mensRows+"</tbody></table>":"")+
